...
${paragraphs}
  ...
  // js/eula-generator.js
  var require_eula_generator = __commonJS({
    "js/eula-generator.js"(exports2, module2) {
      var EulaGenerator = class {
        async validate(input) {
          const normalized = this.normalizeInput(input);
          return {
            ok: normalized.errors.length === 0,
        ...
        normalizeInput(input) {
          ...
          const messages = data.settings.language === "es" ? {
            ...
            missingProductName: "Deb\xE9s indicar el nombre del software, app o producto licenciado.",
            missingSoftwareType: "Conviene indicar qu\xE9 tipo de software o distribuci\xF3n cubre este EULA.",
            missingLicenseGrant: "Deb\xE9s describir la licencia otorgada. Sin eso, el EULA no deja claro qu\xE9 derecho de uso recibe el usuario.",
            ...
            missingInstallationLimit: "Conviene indicar el l\xEDmite de instalaci\xF3n, cuenta, dispositivo o asiento cuando el licenciamiento no es totalmente abierto.",
            ...
            missingWarrantyDisclaimer: 'Conviene incluir un disclaimer de garant\xEDas o explicar si el software se entrega "as is".',
            missingLiabilityLimit: "Conviene resumir la limitaci\xF3n de responsabilidad para que el EULA no quede incompleto frente a da\xF1os indirectos, p\xE9rdida de datos o interrupciones.",
            missingTermination: "Conviene indicar cu\xE1ndo puede terminarse o revocarse la licencia.",
            missingGoverningLaw: "Conviene indicar ley aplicable o foro principal para disputas vinculadas a la licencia.",
            contradictionNoUpdatesWithSupport: "Marcaste que no se proveen updates, pero tambi\xE9n seleccionaste un soporte o mantenimiento que normalmente supone cambios o releases. Revis\xE1 si realmente no hay actualizaciones cubiertas.",
            contradictionTransferableSeatLicense: "Marcaste la licencia como transferible, pero tambi\xE9n describiste un l\xEDmite por asiento o cuenta. Conviene revisar si esa transferencia es real o si la licencia deber\xEDa quedar como no transferible.",
            contradictionNoThirdPartyWithNotice: "Completaste una nota sobre componentes de terceros u open source, pero tambi\xE9n marcaste que no existen. Revis\xE1 cu\xE1l de las dos cosas refleja el producto real."
          } : {
          ...
          const sections = [
            this.section("overview", this.text("Overview", "Resumen"), [
              this.interpolate(this.text(
                ...
                "Este Contrato de Licencia de Usuario Final regula el acceso y uso de {product_name}, distribuido o puesto a disposici\xF3n por {business_name} a trav\xE9s de {website}."
              ), data)
            ]),
            ...
            this.section("termination", this.text("Termination of the License", "Terminaci\xF3n de la licencia"), [
              this.terminationText(data)
            ]),
            this.section("disclaimers", this.text("Warranty Disclaimer and Limitation of Liability", "Descargo de garant\xEDas y limitaci\xF3n de responsabilidad"), [
        ...
        interpolate(text, data) {
          return String(text || "").replaceAll("{business_name}", data.business.name || "").replaceAll("{website}", data.business.websiteUrl || "").replaceAll("{product_name}", data.eula.productName || this.text("the software", "el software"));
        }
        licenseGrantText(data) {
          ...
          if (data.eula.licenseGrant) {
            parts.push(`${this.text("License language", "Redacci\xF3n de la licencia")}: ${this.sentence(data.eula.licenseGrant)}`);
          }
          ...
          if (data.eula.installationLimit) {
            parts.push(`${this.text("Installation or seat limits", "L\xEDmites de instalaci\xF3n o asientos")}: ${this.sentence(data.eula.installationLimit)}`);
          }
          parts.push(this.text(
            data.eula.allowsCommercialUse ? "Commercial or business use is permitted only within the scope described by the service, subscription, or contract tier selected by the customer." : "Commercial or business use is not granted unless a separate commercial plan, contract, or written authorization expressly allows it.",
            data.eula.allowsCommercialUse ? "El uso comercial o empresarial est\xE1 permitido \xFAnicamente dentro del alcance previsto por el servicio, suscripci\xF3n o nivel contractual contratado." : "No se concede uso comercial o empresarial salvo que exista un plan comercial, contrato o autorizaci\xF3n escrita que lo permita de forma expresa."
          ));
          parts.push(this.text(
            data.eula.transferable ? "The license may be transferred only where that transfer is technically and contractually supported by the product or subscription model." : "The license is personal, limited, and non-transferable unless a separate written authorization states otherwise.",
            data.eula.transferable ? "La licencia s\xF3lo podr\xE1 transferirse cuando esa transferencia est\xE9 admitida t\xE9cnica y contractualmente por el producto o la suscripci\xF3n." : "La licencia es personal, limitada e intransferible salvo autorizaci\xF3n escrita en contrario."
          ));
          return parts.join(" ");
        }
        restrictionsText(data) {
          const restrictions = [
            this.text(
              ...
              "El usuario final no puede copiar, sublicenciar, distribuir, alquilar o explotar comercialmente el software m\xE1s all\xE1 de los derechos expresamente otorgados."
            )
          ];
          if (data.eula.reverseEngineeringRestricted) {
            restrictions.push(this.text(
              ...
              "La ingenier\xEDa inversa, descompilaci\xF3n o intentos de derivar c\xF3digo fuente est\xE1n restringidos salvo que la ley obligatoria disponga lo contrario."
            ));
          }
          if (data.eula.modificationRestricted) {
            restrictions.push(this.text(
              ...
              "La modificaci\xF3n o creaci\xF3n de obras derivadas est\xE1 restringida salvo que la licencia aplicable, la documentaci\xF3n o una autorizaci\xF3n escrita lo permitan."
            ));
          }
          if (data.eula.redistributionRestricted) {
            restrictions.push(this.text(
              ...
              "La redistribuci\xF3n, reempaquetado o republicaci\xF3n del software o de sus componentes principales est\xE1 restringida salvo autorizaci\xF3n expresa."
            ));
          }
        ...
        updatesSupportText(data) {
          const parts = [
            this.text(
              data.eula.updatesProvided ? "Updates, patches, or new versions may be made available according to the product roadmap, subscription tier, and operational priorities of the licensor." : "The licensor does not promise future updates, patches, or new versions unless a separate commercial commitment says otherwise.",
              data.eula.updatesProvided ? "Las actualizaciones, parches o nuevas versiones pueden ponerse a disposici\xF3n seg\xFAn el roadmap del producto, el nivel de suscripci\xF3n y las prioridades operativas del licenciante." : "El licenciante no promete futuras actualizaciones, parches o nuevas versiones salvo que exista un compromiso comercial separado."
            )
          ];
        ...
        thirdPartyText(data) {
          if (!data.eula.thirdPartyComponents && !data.eula.openSourceNotice) {
            return this.text(
              ...
              "Pueden existir componentes de terceros u open source en la base t\xE9cnica, pero este acuerdo no ampl\xEDa sus licencias originales y los usuarios deber\xEDan revisar adem\xE1s los avisos que se entreguen con el software."
            );
          }
          return [
            this.text(
            ...
            data.eula.openSourceNotice ? `${this.text("Third-party/open-source notice", "Aviso de terceros/open source")}: ${this.sentence(data.eula.openSourceNotice)}` : ""
          ].filter(Boolean).join(" ");
        }
        terminationText(data) {
          const parts = [
            this.text(
              ...
              "La licencia podr\xE1 terminar autom\xE1ticamente si el usuario final incumple materialmente este acuerdo, usa el software fuera del alcance concedido o viola condiciones de pago, cuenta o cumplimiento vinculadas al producto."
            )
          ];
          if (data.eula.terminationTriggers) {
            parts.push(`${this.text("Typical termination triggers", "Supuestos t\xEDpicos de terminaci\xF3n")}: ${this.sentence(data.eula.terminationTriggers)}`);
          }
          return parts.join(" ");
        }
        disclaimerText(data) {
          ...
          if (data.eula.warrantyDisclaimer) {
            parts.push(`${this.text("Warranty disclaimer", "Descargo de garant\xEDas")}: ${this.sentence(data.eula.warrantyDisclaimer)}`);
          } else {
            parts.push(this.text(
              ...
              'Salvo garant\xEDa comercial separada y expresa, el software se entrega "tal cual" y "seg\xFAn disponibilidad", en la m\xE1xima medida permitida por la ley.'
            ));
          }
          if (data.eula.liabilityLimit) {
            parts.push(`${this.text("Limitation of liability", "Limitaci\xF3n de responsabilidad")}: ${this.sentence(data.eula.liabilityLimit)}`);
          }
          return parts.join(" ");
        }
        governingLawText(data) {
          return data.eula.governingLaw ? `${this.text("Governing law or main dispute forum", "Ley aplicable o foro principal")}: ${this.sentence(data.eula.governingLaw)}` : this.text(
            ...
            "Las partes deber\xEDan revisar la ley aplicable y el foro de disputas que mejor encajen con la distribuci\xF3n y geograf\xEDa de usuarios del software."
          );
        }
        contactText(data) {
          const parts = [];
        ...
        softwareTypeLabel(value) {
          const labels = {
            ...
            mobile_app: this.text("mobile app", "app m\xF3vil"),
            ...
            plugin_extension: this.text("plugin, extension, or add-on", "plugin, extensi\xF3n o add-on")
          };
          return labels[value] || value;
        }
        licenseScopeLabel(value) {
          const labels = {
            personal_internal: this.text("personal or internal business use only", "uso personal o interno \xFAnicamente"),
            commercial_b2b: this.text("commercial business use under the contracted plan", "uso comercial bajo el plan contratado"),
            single_device: this.text("single device or installation scope", "alcance limitado a un dispositivo o instalaci\xF3n"),
        ...
        supportLevelLabel(value) {
          const labels = {
            none: this.text("no support commitment beyond general availability", "sin compromiso de soporte m\xE1s all\xE1 de la disponibilidad general"),
...
${paragraphs}
  ...
  var require_browser_entry = __commonJS({
    "web/browser-entry.js"() {
      ...
      var EulaGenerator = require_eula_generator();
